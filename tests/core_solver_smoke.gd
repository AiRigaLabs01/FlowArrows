...
func _initialize() -> void:
	...
	_test_bent_piece_can_exit()
	_test_bent_piece_blocked_on_head_path()
	_test_thread_slide_avoids_rigid_translation_collision()
...
func _test_bent_piece_can_exit() -> void:
	...
	assert(board.exit_steps("A") > 0)
	assert(SolverScript.new().solve(board) == ["A"])

func _test_bent_piece_blocked_on_head_path() -> void:
	var a = PieceScript.new("A", [Vector2i(0, 1), Vector2i(1, 1), Vector2i(1, 2)], Vector2i.RIGHT)
...
func _test_thread_slide_avoids_rigid_translation_collision() -> void:
	var a = PieceScript.new("A", [Vector2i(0, 1), Vector2i(1, 1), Vector2i(1, 2)], Vector2i.RIGHT)
	var blocker = PieceScript.new("B", [Vector2i(2, 1)], Vector2i.UP)
	var board = BoardScript.new(4, 4, [a, blocker])
	assert(board.can_exit("A"))
